...
func skipTests() bool {
	...
	noTestFittedCloud, _ := strconv.ParseBool(os.Getenv("TEST_SKIP_FITTEDCLOUD"))
	return travis || noTestFittedCloud
}
...
func TestConfig(t *testing.T) {
	...
	tf := func(config gofig.Config, client types.Client, t *testing.T) {
		assert.Equal(t, config.GetString(fittedcloud.ConfigEBSTag), "RR")
	}
	apitests.Run(t, fittedcloud.Name, configYAML, tf)
}
...
func TestInstanceID(t *testing.T) {
	...
	sd, err := registry.NewStorageDriver(fittedcloud.Name)
	if err != nil {
		t.Fatal(err)
	}
	...
	iid, err := fcUtils.InstanceID(ctx)
	...
	apitests.Run(
		t, fittedcloud.Name, nil,
		(&apitests.InstanceIDTest{
			Driver:   fittedcloud.Name,
			Expected: iid,
		}).Test)
}
...
func TestVolumes(t *testing.T) {
	...
	tf := func(config gofig.Config, client types.Client, t *testing.T) {
		...
		_ = volumeDetach(t, client, vol1.ID)
		volumeRemove(t, client, vol1.ID)
		_ = volumeDetach(t, client, vol2.ID)
...
func volumeCreate(
	...
	reply, err := client.API().VolumeCreate(nil, fittedcloud.Name, volumeCreateRequest)
	assert.NoError(t, err)
	if err != nil {
		t.FailNow()
	...
	fcSize := (size + fcagent.FcVolMaxCloudDisk - 1) / fcagent.FcVolMaxCloudDisk
	assert.Equal(t, fcSize, reply.Size)
	return reply
}
...
func volumeByName(
	t *testing.T, client types.Client, volumeName string) *types.Volume {
	log.WithField("volumeName", volumeName).Info("get volume by fittedcloud.Name")
	...
	assert.Contains(t, vols, fittedcloud.Name)
	for _, vol := range vols[fittedcloud.Name] {
...
func volumeRemove(t *testing.T, client types.Client, volumeID string) {
	...
	err := client.API().VolumeRemove(
		nil, fittedcloud.Name, volumeID, false)
	assert.NoError(t, err)
...
func volumeAttach(
	...
	nextDevice, err := client.Executor().NextDevice(context.Background().WithValue(context.ServiceKey, fittedcloud.Name),
		utils.NewStore())
	assert.NoError(t, err)
	if err != nil {
	...
	reply, token, err := client.API().VolumeAttach(
		nil, fittedcloud.Name, volumeID, &types.VolumeAttachRequest{
...
func volumeInspect(
	...
	reply, err := client.API().VolumeInspect(nil, fittedcloud.Name, volumeID, 0)
	assert.NoError(t, err)
...
func volumeDetach(
	...
	reply, err := client.API().VolumeDetach(
		nil, fittedcloud.Name, volumeID, &types.VolumeDetachRequest{})
	assert.NoError(t, err)
	if err != nil {
